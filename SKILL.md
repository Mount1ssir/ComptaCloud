...
Next.js 16.2.6 (App Router, Turbopack, SSR cookie sessions via proxy.ts).
PostgreSQL 15 / Supabase Local Docker with Row Level Security (RLS) & SECURITY DEFINER RPC functions.
Dynamic RBAC & Plan Quotas: public.roles, public.permissions, public.role_permissions, public.plans, public.plan_permissions, public.plan_limits.
Security Primitives: is_super_admin(), is_platform_role(), has_permission(), has_plan_permission(), check_plan_limit(), and can_perform_with_plan().
2. Completed Features
Tenant Onboarding: Atomic creation via create_tenant_with_admin_invite and pending-to-active transition upon first login.
Dynamic Roles & Permissions (Phases 0–F): Dynamic roles with platform-role RLS isolation (authenticated_select_roles).
Plans & Enforcement (Phases 0–E.2.4):
Backfilled subscriptions with audit trail.
Auto-generated permissions reference lib/permissions.gen.ts with CI drift enforcement.
Converted /api/drive/connect (E.2.1), /api/drive/callback (E.2.2), disconnectDriveAction (E.2.3), and save_tenant_drive_token RPC (E.2.4) to can_perform_with_plan.
3. Dedicated Test Accounts
Super Admin: walkthrough.superadmin@cabinetsplatform.com / Password123!
Cabinet Admin: user.admin@cabinet.com / Password123! (Cabinet User Platform, Pro plan)
Accountant: user.accountant@cabinet.com / Password123!
Client: user.client@cabinet.com / Password123!
4. Current Status & Next Steps
Current Issues / Lints: 0 Errors. TypeScript compiler (npx tsc --noEmit) and all 15 regression scripts pass cleanly.
Immediate Next Steps:
Step E.2.5: Convert tenant_isolation_tenants_update_drive RLS policy on public.tenants (the 5th and final drive:connect call site) to can_perform_with_plan('drive:connect').
Step E.3: Wire check_plan_limit('max_accountants') into inviteStaffAction in app/dashboard/actions.ts.
Step E.4: Add UI quota indicators and plan upgrade alerts to /dashboard/team.