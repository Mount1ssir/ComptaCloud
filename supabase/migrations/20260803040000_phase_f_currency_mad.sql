-- Migration: Phase F Adjustment 1 — Currency MAD Default & RPC Update

-- 1. Alter plans default currency to MAD
ALTER TABLE public.plans 
ALTER COLUMN currency SET DEFAULT 'MAD';

-- 2. Update existing plans currency to MAD
UPDATE public.plans 
SET currency = 'MAD';

-- 3. Update upsert_plan_details RPC to default p_currency to 'MAD'
...
BEGIN
    ...
    IF v_target_plan_id IS NULL THEN
        ...
        VALUES (TRIM(p_name), LOWER(TRIM(p_slug)), p_description, p_price_monthly, COALESCE(NULLIF(TRIM(p_currency), ''), 'MAD'), p_tier_rank, COALESCE(p_is_active, true))
    ...
    ELSE
        ...
        SET name = TRIM(p_name),
            ...
            currency = COALESCE(NULLIF(TRIM(p_currency), ''), 'MAD'),
